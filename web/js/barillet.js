var s,
    smallCarousel = {
        setting: {
            ...
            $bioCollaborateur: $('.bioCollaborateur'),
            $bioCollaborateurItems: $('.bioCollaborateur > li'),
            $current: 0,
            $isAuto: true,
            $acAuto: 3000
        },
        ...
        /*  addItems: function() {
            var newContainer = '<div class="colBarillet"><ul class="bioCollaborateur"></ul></div>';
            var newCounter = '';
            s.$wrapper.append(newContainer);
            for (var i = 0; i < s.$_nodes; i++) {
              newCounter += ('<li><a href="#">' + (i+1) + '</a></li>');
            }
            s.$bioCollaborateur.append(newCounter);
            s.$bioCollaborateur.find('li').eq(0).addClass('active');
          },*/
        ...
        initEvents: function () {
            ...
            $.each(s.$bioCollaborateurItems, function (i) {
                var $this = $(this);
                $this.find('a').on(clickEvent, function (e) {
                    e.preventDefault();
                    clearCarousel(i);
        ...
        getRotate: function (obj, pos) {
            ...
            s.$bioCollaborateurItems.find('a').not(s.$bioCollaborateurItems.eq(pos)).removeClass('active');
            s.$bioCollaborateurItems.eq(pos).find('a').not().addClass('active');
        }
    };
